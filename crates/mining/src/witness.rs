...
pub const COINBASE_WTXID_PLACEHOLDER: Hash256 = Hash256::ZERO;
...
pub fn commitment(witness_root: Hash256, reserved_value: &[u8; 32]) -> Hash256 {
...
pub fn commitment_script(wtxids: &[Hash256]) -> Vec<u8> {
...
mod tests {
    ...
    fn script_is_always_thirty_eight_bytes() {
        for count in 0..8 {
            let wtxids: Vec<_> = (0..count).map(|n| Hash256::sha256d(&[n as u8])).collect();
    ...
    fn commitment_depends_on_the_wtxids() {
        let a = commitment_script(&[Hash256::sha256d(b"one")]);
        let b = commitment_script(&[Hash256::sha256d(b"two")]);
        assert_ne!(a, b);
    }
}
