...
impl Lifetime {
    ...
    pub fn load(path: &Path) -> Self {
        let mut lifetime = std::fs::read_to_string(path)
            ...
            .unwrap_or_else(|| Self {
                total_hashes: 0,
                best_hash: Hash256::from_internal_bytes([0xFF; 32]).to_string(),
                best_zero_bits: 0,
                sessions: 0,
                first_run: now(),
                last_run: now(),
    ...
    pub fn record(&mut self, new_hashes: u64, best: Hash256, best_zero_bits: u32) {
...
pub fn default_path() -> PathBuf {
    std::env::var("HOME")
        .map(|home| PathBuf::from(home).join(".bip110-miner").join("lifetime.json"))
...
mod tests {
    ...
    fn totals_survive_a_round_trip() {
        ...
        first.record(1_000_000, Hash256::sha256d(b"a"), 33);
    ...
    fn best_never_regresses() {
        ...
        lifetime.record(10, Hash256::sha256d(b"good"), 40);
        let good = lifetime.best_hash.clone();

        lifetime.record(10, Hash256::sha256d(b"worse"), 12);
    ...
    fn odds_scale_linearly_with_work() {
        ...
        let mut lifetime = Lifetime::load(&path);
        lifetime.record(1_000_000_000_000, Hash256::sha256d(b"x"), 30);

        let difficulty = 1.142e9;
