...
impl HttpClient {
    ...
    ///
    /// `path` is almost always `/`. The exception is a wallet RPC on a node
    /// with more than one wallet loaded, which must be addressed as
    /// `/wallet/<name>` — see [`RpcClient::call_wallet`](crate::RpcClient::call_wallet).
    pub fn post_json(
        &self,
        path: &str,
        authorization: &str,
