...
impl RpcClient {
    ...
    ///
    /// # Why the port can be overridden
    ///
    ...
    pub fn from_datadir(datadir: &Path, network: Network) -> Result<Self, RpcError> {
        Self::connect(datadir, network, rpc_port_override().unwrap_or(network.default_rpc_port()))
    }
    ...
    ///
    /// Wallet RPCs are dispatched by URI, not by argument. With exactly one
    /// wallet loaded the node will guess, and a request to `/` works — which
    /// is how this is easy to get wrong, because it works right up until the
    /// day a second wallet is loaded and then fails with:
    ///
    /// ```text
    ...
    /// ```
    ///
    /// Naming the wallet explicitly is also the safer behaviour on its own
    /// terms: a miner that let the node guess could be handed a payout address
    /// from a wallet the user did not mean to mine into.
